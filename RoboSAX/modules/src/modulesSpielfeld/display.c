...
#define disp_row_ser(x)                          \
    (x ? (DISP_ROW_SER_PORT |= DISP_ROW_SER_PIN) \
...
#define disp_row_sck(x)                          \
    (x ? (DISP_ROW_SCK_PORT |= DISP_ROW_SCK_PIN) \
...
#define disp_rck(x) \
    (x ? (DISP_RCK_PORT |= DISP_RCK_PIN) : (DISP_RCK_PORT &= ~DISP_RCK_PIN))
...
#define disp_enable(x)                         \
    (x ? (DISP_ENABLE_PORT |= DISP_ENABLE_PIN) \
...
#define disp_col_ser(x)                          \
    (x ? (DISP_COL_SER_PORT |= DISP_COL_SER_PIN) \
...
#define disp_col_sck(x)                          \
    (x ? (DISP_COL_SCK_PORT |= DISP_COL_SCK_PIN) \
...
#define display_working_supersegmentbuffer \
    display_supersegmentbuffer[display_current_working_buffer]
#define display_showing_supersegmentbuffer \
    display_supersegmentbuffer[display_current_display_buffer]
...
void display_setSegment(const uint8_t *pict, uint8_t segmentnumber) {
    if ((segmentnumber >= 0) && (segmentnumber < SEGMENTS_COUNT)) {
        ...
        for (i = 0; i < 7; i++) {
            ...
            display_working_supersegmentbuffer[superSeg][i] |= (pict[i] & 0x1F)
                                                               << posLast;
        }
        displayNeedsUpdate = 1;
    }
...
void display_setSuperSegment(const uint16_t *pict, uint8_t supersegmentnumber) {
    if ((supersegmentnumber >= 0) &&
        (supersegmentnumber < SUPER_SEGMENTS_COUNT)) {
        int8_t  i;
        ...
        if ((hight <= SUPER_SEGMENT_HIGHT) && (width <= SUPER_SEGMENT_WIDTH)) {
            display_working_supersegmentbuffer[supersegmentnumber]
                                              [SUPER_SEGMENT_HIGHT] =
                                                pict[SUPER_SEGMENT_HIGHT];
            for (i = 0; i < SUPER_SEGMENT_HIGHT; i++) {
                display_working_supersegmentbuffer[supersegmentnumber][i] =
                  pict[i] << (16 - width);
            }
        } else {
            for (i = 0; i < 8; i++) {
...
void display_addLeft(const uint8_t *pict, uint8_t supersegmentnumber,
                     uint8_t sparse) {
    if ((supersegmentnumber >= 0) &&
        (supersegmentnumber < SUPER_SEGMENTS_COUNT)) {
        int8_t  i;
        ...
        uint8_t oldwidth =
          0x00FF & display_working_supersegmentbuffer[supersegmentnumber]
                                                     [SUPER_SEGMENT_HIGHT];
        uint8_t oldhight =
          0xFF00 & display_working_supersegmentbuffer[supersegmentnumber]
                                                     [SUPER_SEGMENT_HIGHT] >>
                     8;

        uint8_t movewidth =
          (sparse && oldwidth && (oldwidth + width + 1 <= SUPER_SEGMENT_WIDTH))
            ? 1
            : 0;
        ...
        if ((newhight <= SUPER_SEGMENT_HIGHT) &&
            (newwidth <= SUPER_SEGMENT_WIDTH)) {
            display_working_supersegmentbuffer[supersegmentnumber]
                                              [SUPER_SEGMENT_HIGHT] =
                                                (((uint16_t)(newhight)) << 8) |
                                                (newwidth);
            for (i = 0; i < SUPER_SEGMENT_HIGHT; i++) {
                display_working_supersegmentbuffer[supersegmentnumber][i] =
                  ...
                  movewidth;
                display_working_supersegmentbuffer[supersegmentnumber][i] |=
                  ((uint16_t)(pict[i]) & ((1 << width) - 1)) << (16 - width);
            }
        }
        displayNeedsUpdate = 1;
    }
...
void display_addRight(const uint8_t *pict, uint8_t supersegmentnumber,
                      uint8_t sparse) {
    if ((supersegmentnumber >= 0) &&
        (supersegmentnumber < SUPER_SEGMENTS_COUNT)) {
        int8_t  i;
...
// void display_addAbove(const uint8_t *pict,uint8_t supersegmentnumber,uint8_t
// sparse){} void display_addBelow(const uint8_t *pict,uint8_t
// supersegmentnumber,uint8_t sparse){} void display_windowShiftright(uint8_t
// number); void display_windowShiftleft (uint8_t number);
...
void display_fill_current_row(void) {
    if (display_current_row == SUPER_SEGMENT_HIGHT) {
        // double dot is out -> dont show it by shift row out, next is anyhow
        // newstart
...
void display_switchBuffer() {
    ...
    if (displayNeedsUpdate)
        if (systick_freezUpdate(update_Display)) {
            display_current_display_buffer = 0;
            ...
            for (i = 0; i < SUPER_SEGMENTS_COUNT; i++)
                for (j = 0; j < 8; j++)
                    display_showing_supersegmentbuffer[i][j] =
                      display_working_supersegmentbuffer[i][j];
