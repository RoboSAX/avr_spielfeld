...
#define bus_btn_load(x)                          \
    (x ? (BUS_BTN_LOAD_PORT |= BUS_BTN_LOAD_PIN) \
...
#define bus_led_data(x)                          \
    (x ? (BUS_LED_DATA_PORT |= BUS_LED_DATA_PIN) \
...
#define bus_led_load(x)                          \
    (x ? (BUS_LED_LOAD_PORT |= BUS_LED_LOAD_PIN) \
...
#define bus_ir_clk(x)                        \
    (x ? (BUS_IR_CLK_PORT |= BUS_IR_CLK_PIN) \
...
#define bus_clk(x) \
    (x ? (BUS_CLK_PORT |= BUS_CLK_PIN) : (BUS_CLK_PORT &= ~BUS_CLK_PIN))
...
#define bus_rgb(x) \
    (x ? (BUS_RGB_PORT &= ~BUS_RGB_PIN) : (BUS_RGB_PORT |= BUS_RGB_PIN))
...
#define return_btn(x)                        \
    (x ? (RETURN_BTN_PORT |= RETURN_BTN_PIN) \
...
void ledbox_init(void) {
    ...
    // bus
    _bus_btn_data_init();
    _bus_btn_load_enable();
    bus_btn_load(0);

    _bus_led_data_enable();
    bus_led_data(0);
    _bus_led_load_enable();
    bus_led_load(0);
    ...
    UCSR0C = _BV(UCSZ01);  // 7 bits in UART frame (UCSZ0x = 0x010)
                           ...
                           //_BV(UCSZ00)  // 7 bits in UART frame (UCSZ0x =
                           // 0x010) _BV(UCPOL0)  // clock polarity (not used)
...
void ledbox_setup_module_count(void) {
    ...
    while (!state) {
        ...
        if (ledbox_count_current > LEDBOX_COUNT_MAX) break;
...
void _ledbox_switchBuffer() {
    ...
    if (needsUpdate & update_buttons)
        if (systick_freezUpdate(update_buttons)) {
            ...
            for (number = 0; number < LEDBOX_COUNT_MAX; number++) {
                ...
                ledbox_buttons[number].flankRead |=
                  ledbox_buttons[number].flankWrite;
    ...
    if ((needsUpdate & update_RGBLeds) &&
        (systick_freezUpdate(update_RGBLeds))) {
        uint8_t number;
        for (number = 0; number < LEDBOX_COUNT_MAX; number++) {
            ...
            if ((d & 0x80) == 0x00) {
                ledbox_rgb[number].dataOut[0] |= 0x01;
            }
            if ((d & 0x40) == 0x00) {
                ledbox_rgb[number].dataOut[0] |= 0x08;
            }
            if ((d & 0x20) == 0x00) {
                ledbox_rgb[number].dataOut[0] |= 0x40;
            }

            if ((d & 0x10) == 0x00) {
                ledbox_rgb[number].dataOut[1] |= 0x01;
            }
            if ((d & 0x08) == 0x00) {
                ledbox_rgb[number].dataOut[1] |= 0x08;
            }
            if ((d & 0x04) == 0x00) {
                ledbox_rgb[number].dataOut[1] |= 0x40;
            }

            if ((d & 0x02) == 0x00) {
                ledbox_rgb[number].dataOut[2] |= 0x01;
            }
            if ((d & 0x01) == 0x00) {
                ledbox_rgb[number].dataOut[2] |= 0x08;
            }
            d = ledbox_rgb[number].r;
            if ((d & 0x80) == 0x00) {
                ledbox_rgb[number].dataOut[2] |= 0x40;
            }

            if ((d & 0x40) == 0x00) {
                ledbox_rgb[number].dataOut[3] |= 0x01;
            }
            if ((d & 0x20) == 0x00) {
                ledbox_rgb[number].dataOut[3] |= 0x08;
            }
            if ((d & 0x10) == 0x00) {
                ledbox_rgb[number].dataOut[3] |= 0x40;
            }

            if ((d & 0x08) == 0x00) {
                ledbox_rgb[number].dataOut[4] |= 0x01;
            }
            if ((d & 0x04) == 0x00) {
                ledbox_rgb[number].dataOut[4] |= 0x08;
            }
            if ((d & 0x02) == 0x00) {
                ledbox_rgb[number].dataOut[4] |= 0x40;
            }

            if ((d & 0x01) == 0x00) {
                ledbox_rgb[number].dataOut[5] |= 0x01;
            }
            d = ledbox_rgb[number].b;
            if ((d & 0x80) == 0x00) {
                ledbox_rgb[number].dataOut[5] |= 0x08;
            }
            if ((d & 0x40) == 0x00) {
                ledbox_rgb[number].dataOut[5] |= 0x40;
            }

            if ((d & 0x20) == 0x00) {
                ledbox_rgb[number].dataOut[6] |= 0x01;
            }
            if ((d & 0x10) == 0x00) {
                ledbox_rgb[number].dataOut[6] |= 0x08;
            }
            if ((d & 0x08) == 0x00) {
                ledbox_rgb[number].dataOut[6] |= 0x40;
            }

            if ((d & 0x04) == 0x00) {
                ledbox_rgb[number].dataOut[7] |= 0x01;
            }
            if ((d & 0x02) == 0x00) {
                ledbox_rgb[number].dataOut[7] |= 0x08;
            }
            if ((d & 0x01) == 0x00) {
                ledbox_rgb[number].dataOut[7] |= 0x40;
            }
        }
        needsUpdate &= ~update_RGBLeds;
