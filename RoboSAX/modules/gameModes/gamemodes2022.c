...
uint8_t gamemode_start_2022(uint8_t gameMode, enum eOperationModes oM,
...
void gamemode_to_display_2022(uint8_t gameMode, uint8_t const** displayOut1,
    ...
    for (uint8_t i = 0; i < ledbox_count_current; i++) {
        if (LEDs[i].on) {
            ...
            if (LEDs[i].blinking.numberOfBlinks > 0) {
                ...
                if (LEDs[i].blinking.countdown)
                    LEDs[i].blinking.countdown--;
