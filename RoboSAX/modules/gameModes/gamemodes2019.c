...
enum eTeamNumber { team1, team2, teamNeutral };
                    ...
                    if (buttons_get(j)) {
                        if(LEDToTeam[j].teamNr >= teamNr){
                            ...
                            for(reCheck=0;reCheck<LED_PER_TEAM;reCheck++){
                                ...
                                if (team[teamNr].LEDs[reCheck].globalNumber >= LEDBOX_COUNT_MAX){ freeNumber =
       reCheck;break;}
                            }
...
uint8_t gamemode_start_2019(uint8_t              gameMode,
...
void gamemode_update_2019() {
    ...
    setLEDs();
    team[teamNeutral].points =
      team[teamNeutral].disabledLeds * POINTSFORNEUTRAL;

    team[team1].points = team[team1].disabledLeds
                           ? team[team1].disabledLeds * POINTSFOROWNCOLOR +
                               team[teamNeutral].points
                           : 0;
    team[team2].points = team[team2].disabledLeds
                           ? team[team2].disabledLeds * POINTSFOROWNCOLOR +
                               team[teamNeutral].points
                           : 0;
...
struct Points gamemode_points_2019(uint8_t mode) {
    ...
    if (mode % 2) {
        ...
        points.maxPoints =
          (PHASE0 * (POINTSFORNEUTRAL + POINTSFOROWNCOLOR)) - 1U;
    } else {
...
void setLEDs(void) {
    ...
    for (teamNr = 0; teamNr < 3; teamNr++) {
        ...
        for (i = 0; i < LED_PER_TEAM; i++) {
            ...
            if (number < LEDBOX_COUNT_MAX) {
                ...
                if (team[teamNr].correct.LEDNr == i) {
                    ...
                    if (!global_blink_countdown) {
                        if (team[teamNr].correct.countdown)
                            team[teamNr].correct.countdown--;
                ...
                if (team[teamNr].error.LEDNr == i) {
                    ...
                    if (!global_blink_countdown) {
                        if (team[teamNr].error.countdown)
                            team[teamNr].error.countdown--;
        ...
        if (!global_blink_countdown) {
            if (team[teamNr].fullBlink.countdown)
                team[teamNr].fullBlink.countdown--;
...
static void calculateLEDNumber(volatile struct sTeam *team) {
    if (team->disabledLeds >= PHASE0)
        team->numberOfActivLeds = 0;
    else if (team->disabledLeds >= PHASE1)
        team->numberOfActivLeds = 1;
    else if (team->disabledLeds >= PHASE2)
        team->numberOfActivLeds = 2;
    else if (team->disabledLeds >= PHASE3)
        team->numberOfActivLeds = 3;
    else if (team->disabledLeds >= PHASE4)
        team->numberOfActivLeds = 4;
    else if (team->disabledLeds >= PHASE5)
        team->numberOfActivLeds = 5;
    else if (team->disabledLeds >= PHASE6)
        team->numberOfActivLeds = 6;
    else if (team->disabledLeds >= PHASE7)
        team->numberOfActivLeds = 7;
...
void pushButton(uint8_t number) {
    ...
    if (team[teamNr].LEDs[LEDNr].activ) {
        ...
        for (i = STOREDLASTLEDS - 1; i > 0; i--) {
            team[teamNr].lastPressedLeds[i] =
              team[teamNr].lastPressedLeds[i - 1];
        }
...
void diasbleOneLEDForTeam(volatile struct sTeam *team) {
    ...
    uint8_t i;
    for (i = 0; i < LED_PER_TEAM; i++) {
        if ((team->LEDs[i].activ) &&
            (team->LEDs[i].globalNumber != LEDBOX_COUNT_MAX)) {
            activLEDs |= 0x01 << i;
            numActivLEDs++;
        }
    }
...
void setLEDForTeam(volatile struct sTeam *team) {
    ...
    for (i = 0;
         (i < STOREDLASTLEDS) && (!deactivateLastLed || (i < ledsToSwitch / 2));
         ++i) {
...
void gamemode_to_display_2019(uint8_t gameMode, uint8_t const **displayOut1,
