...
void update () {
	...
	uint8_t trys;
	enum eUpdate updated=0;
	for (trys=0;trys<2;trys++){
		if(!(updated & update_Display) && (makeUpdate & update_Display)){
			display_show();
			updated|=update_Display;
		}
		if(!(updated & update_RGBLeds) && (makeUpdate & update_RGBLeds)){
			_ledbox_rgb_update();
			updated|=update_RGBLeds;
		}
		if(!(updated & update_buttons) && (makeUpdate & update_buttons)){
			_ledbox_buttons_and_ir_update();
			_master_buttons_update();
			updated|=update_buttons;
		}
	}
	makeUpdate&=~update_activ;
}
