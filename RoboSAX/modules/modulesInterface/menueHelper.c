...
void pointLedMagic(struct Points points, uint8_t count) {
    ...
    if (points.type == ptTeam1) {
        showOnePoints(points.team1);
        showOnePointsLed(points.team1, points.color1, points.maxPoints, number);
    } else {
        showPoints(points.team1, points.team2);
        if (points.type == ptBeide) {
            showPointsLed(points.team1, points.color1, points.team2,
                          points.color2, points.maxPoints, number);
        } else {
            showPointsLedVs(points.team1, points.color1, points.team2,
                            points.color2, points.maxPoints, number);
        }
    }
}

void waitAndUpdate() { waitMsAndUpdate(0); }
...
void showPointsLed(uint8_t team1Points, enum eColor color1, uint8_t team2Points,
    ...
    if (numberLeds > ledbox_count_current / 2)
        numberLeds = ledbox_count_current / 2;
...
void writeModesToDisplay(enum eMasterModes masterMode, int8_t subMode) {
	...
	break;
        ...
        default: {
            uint8_t const* display4Chars[4];
            game_to_display(masterMode - DefaultMasterModes + 1,
                            &display4Chars[0], &display4Chars[1],
                            &display4Chars[2], &display4Chars[3]);
            display_clearSuperSegment(0);
